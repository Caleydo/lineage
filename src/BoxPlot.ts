...
import {max, min, ticks, range, extent, quantile} from 'd3-array';
...
export default class BoxPlot {
  ...
  async init(name, dataVec, type,width,height) {
    ...
    this.xScale = scaleLinear().range([5, width-5]);
    this.yScale = scaleLinear().range([0, height]);
    this.width = width;
    this.height = height;
    this.update(dataVec);
    // this.attachListener();
  ...
  //returns an array with the .25 .5 and .75 quantiles of a sorted array;
  private boxQuartiles(d) {
  	return [
    	quantile(d, .25),
    	quantile(d, .5),
    	quantile(d, .75)
  ...
  private update(dataVec) {
    ...
    // } else
     if (this.type === VALUE_TYPE_INT || this.type === VALUE_TYPE_REAL) {
      ...
      console.log('dataVec is ', dataVec, this.height);
      const xScale = this.xScale;
      const yScale = this.yScale;

      xScale.domain([min(dataVec.dataValues),max(dataVec.dataValues)]);
      yScale.domain([0,1]);
