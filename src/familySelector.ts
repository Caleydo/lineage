...
class FamilySelector {
   ...
   */
  private build() {
    select('#collapseTableButton').on('click', () => {
  ...
  public updateTable() {
    ...
    const headerEnter = headers.enter().append('th');
    ...
    const selectedRows = rowData.filter((row) => {
      return this.selectedFamilyIds.indexOf(row.id) > -1;
    });
    ...
    select('#tableBody')
      ...
      .classed('selected', (d: any) => {
        return this.selectedFamilyIds.indexOf(d.id) > -1;
      });

    selectAll('.addRemoveIcon').on('click', (d) => {
      event.stopPropagation();
      this.selectRow(d, rowData, tableHeaders.length - 2);
    });

    if (selectAll('.selected').size() === 0) {
      console.log('emptyFamily!');
      this.selectRow(
        { id: data.familyInfo[0].id },
        rowData,
  ...
  private selectRow(familyID: any, rowData: any, numCols, update = true) {
    ...
    if (!toRemove) {
      ...
      this.selectedFamilyIds.splice(
        this.selectedFamilyIds.indexOf(familyID.id),
        1
      );
    }
  ...
  private populateTableRows(tableSelector, rowData, numCols) {
    ...
    let cells = rows.selectAll('td').data((d: any) => {
      const baseValues = [
    ...
    cells
      ...
      .html((d: any) => {
        return tableSelector === '#tableHead'
          ? '\uf056'
          : this.selectedFamilyIds.indexOf(d.id) > -1
          ? '\uf056'
          : '\uf055';
      })
      .style('text-align', 'center')
