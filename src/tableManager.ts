...
export const PRIMARY_CATEGORICAL_COLORS = [
  PRIMARY_COLOR,
  PRIMARY_COLOR_2,
  PRIMARY_COLOR_3,
  PRIMARY_COLOR_4,
...
export default class TableManager {
  ...
  public temporalDataInterval = {};
  public temporalDataMeans = {};
  public temporalDataExtreme = {};
  ...
  public async loadData(descendDataSetID: string, attributeDataSetID: string) {
    const self = this;
    if (
      descendDataSetID === 'AllFamiliesDescend' ||
  ...
  public async setPrimaryAttribute(attributeName?) {
    ...
    const attributeDefinition: IPrimaryAttribute = {
      ...
      data: await attributeVector.data(),
      range: attributeVector.desc.value.range,
      personIDs: await attributeVector.names()
    };
    ...
    if (attributeDefinition.type === VALUE_TYPE_CATEGORICAL) {
      ...
      attributeDefinition.type === VALUE_TYPE_INT ||
      attributeDefinition.type === VALUE_TYPE_REAL
    ) {
  ...
  public async updatePOI_Primary() {
    if (this.affectedState) {
      const attributeVector = await this.getAttributeVector(
        this.affectedState.name
      );
    ...
    if (typeof isAffectedCallbackFcn === 'undefined') {
      if (varType === VALUE_TYPE_INT || varType === VALUE_TYPE_REAL) {
        ...
        isAffectedCallbackFcn = (attr: string) => {
          return (
            !isUndefined(attr) && attr.toLowerCase() === category.toLowerCase()
          ); //solve the True/TRUE problem once and for all.
        };
        threshold = category;
  ...
  public async updateFamilyStats() {
    const attributeVector = await this.getAttributeVector(
      this.affectedState.name,
      true
    ); //get Attribute Vector for all families
    ...
    uniqueFamilyIDs.map((familyID, index) => {
      ...
      this.familyInfo[index].percentage =
        affectedDict[familyID] / this.familyInfo[index].size;
    });
   ...
   * This function calculates the number of affected people based on the current POI selected in the panel.
   */
  public async updateFamilySelector(
    attribute: string,
    trueValue: string,
    add: boolean
  ) {
    ...
    if (!add) {
      const allAttributes = this.familyInfo[0].starCols.map((attr: any) => {
    ...
    const poiVector = await this.getAttributeVector(
      this.affectedState.name,
      true
    ); //get POI Vector for all families
    ...
    attributeData.map((dataPoint, ind) => {
      if (dataPoint === trueValue || (!trueValue && dataPoint)) {
        ...
        const poiInd =
          attributeData.length === familyIDs.length
            ? ind
    ...
    uniqueFamilyIDs.map((familyID, index) => {
      ...
      const percentage =
        this.familyInfo[index].affected > 0
          ? starCountDict[familyID] / this.familyInfo[index].affected
          : 0;
      this.familyInfo[index].starCols.push({
        attribute,
  ...
  public async parseFamilyInfo() {
    const familyIDs: number[] = <number[]>(
      await this.table.col(indexOfKindredIDColumn).data()
    ); //Assumes kindredID is the first col. Not ideal.
  ...
  public async refreshActiveTableView() {
    const tableRange = range.join(
      this._activeTableRows,
  ...
  public async refreshActiveGraphView() {
    const graphRange = range.join(
      this._activeGraphRows,
      this.activeGraphColumns
    );
  ...
  public async setActiveRowsWithoutEvent(newRows: string[]) {
    ...
    const allIDs = await this.attributeTable.col(0).names();
  ...
  set activeTableRows(newRows: range.Range) {
    ...
    this.tableTable = this.table.view(
      range.join(this._activeTableRows, this.activeTableColumns)
    );
  ...
  public getAirQualityColumnsNames(aqTable) {
    let colNames = aqTable.cols().map((col) => {
      ...
      for (const item in this.temporalData) {
  ...
  public getEntireAirQualityColumns(attributeName) {
    return this.airqualityTable
      .cols()
      .filter((col) => col.desc.name.includes(attributeName));
  }
