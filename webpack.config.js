...
const buildId = `${now.getUTCFullYear()}${prefix(now.getUTCMonth())}${prefix(
  now.getUTCDate()
)}-${prefix(now.getUTCHours())}${prefix(now.getUTCMinutes())}${prefix(
  now.getUTCSeconds()
)}`;
...
function testPhoveaModule(moduleName, request) {
  ...
  const rootPath = /phovea_.*/.test(moduleName)
    ? ["phovea", moduleName.slice(7)].concat(path.slice(1))
    : path;
  return {
    root: rootPath,
    commonjs2: path,
...
const isWorkspaceContext = fs.existsSync(
  resolve(__dirname, "..", "phovea_registry.js")
);
const registryFile = isWorkspaceContext
  ? "../phovea_registry.js"
  : "./phovea_registry.js";
const actMetaData = `file-loader?name=phoveaMetaData.json!${buildInfo.metaDataTmpFile(
  pkg
)}`;
...
function generateWebpack(options) {
  let base = {
    ...
    output: {
      ...
      filename:
        (options.name || pkg.name + (options.bundle ? "_bundle" : "")) +
        (options.min && !options.nosuffix ? ".min" : "") +
        ".js",
  ...
  if (options.library) {
    let libName = /phovea_.*/.test(pkg.name)
      ? ["phovea", pkg.name.slice(7)]
      : pkg.name;
  ...
  if (!options.bundle) {
    ...
    base.externals.push(
      ...(options.externals || Object.keys(options.libs || {}))
    );
  ...
  if (!options.bundle || options.isApp) {
    ...
    let p = new ExtractTextPlugin({
      filename:
        (options.isApp || options.moduleBundle ? "style" : pkg.name) +
        (options.min && !options.nosuffix ? ".min" : "") +
        ".css",
